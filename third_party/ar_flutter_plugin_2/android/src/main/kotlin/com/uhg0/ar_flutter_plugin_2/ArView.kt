...
class ArView(
    ...
    private val onSessionMethodCall =
        MethodChannel.MethodCallHandler { call, result ->
            when (call.method) {
                ...
                "getPlaneHitAtScreenCenter" -> handleGetPlaneHitAtScreenCenter(result)
    ...
    private fun handleGetPlaneHitAtScreenCenter(result: MethodChannel.Result) {
        try {
            ...
            val serializedHit = serializeHitResult(
                mapOf(
                    "type" to 1,
                    "distance" to hit.distance.toDouble(),
                    "position" to mapOf(
            ...
            result.success(serializedHit)
        } catch (e: Exception) {
            result.error("PLANE_HIT_TEST_ERROR", e.message, null)
        }
    }
    ...
    private fun handleGetCameraPose(result: MethodChannel.Result) {
        try {
            ...
            if (cameraPose != null) {
                result.success(serializePose(cameraPose).toList())
            } else {
    ...
    private fun handleGetAnchorPose(
        ...
        try {
            ...
            if (anchorPose != null) {
                result.success(serializePose(anchorPose).toList())
            } else {
                result.error("ANCHOR_NOT_FOUND", "Anchor with ID $anchorId not found", null)
            }
        } catch (e: Exception) {
