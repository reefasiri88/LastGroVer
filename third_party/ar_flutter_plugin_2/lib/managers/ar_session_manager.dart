...
class ARSessionManager {
  ...
  /// Returns the first valid tracked horizontal-plane hit result at the center of the screen.
  Future<ARHitTestResult?> getPlaneHitAtScreenCenter() async {
    try {
      final serializedHit = await _channel.invokeMethod<Map<dynamic, dynamic>>(
        'getPlaneHitAtScreenCenter',
      );
      if (serializedHit == null) {
        return null;
      }
      return ARHitTestResult.fromJson(
        Map<String, dynamic>.from(serializedHit),
      );
    } catch (e) {
      print('Error caught: ' + e.toString());
      return null;
